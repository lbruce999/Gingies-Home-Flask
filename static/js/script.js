var gingiesRoutes = window.GingiesRoutes || {};
var LOGIN_PAGE_PATH = gingiesRoutes.login || "loginPage.html";
var DASHBOARD_PAGE_PATH = gingiesRoutes.dashboard || "dashboard.html";
var FORGOT_PASSWORD_PAGE_PATH = gingiesRoutes.forgotPassword || "forgotpwd.html";
...
function initSiteNav() {
  ...
  if (!navToggle || !siteNav || !window.bootstrap) {
    return;
  }

  siteNav.addEventListener("shown.bs.collapse", function () {
  ...
  siteNav.querySelectorAll("a").forEach(function (link) {
    link.addEventListener("click", function () {
      if (window.innerWidth <= 768) {
        window.bootstrap.Collapse.getOrCreateInstance(siteNav, { toggle: false }).hide();
      }
    });
  });
}
...
function initQuoteForm() {
  ...
  quoteForm.addEventListener("submit", async function (event) {
    ...
    try {
      ...
      setFormMessage(formMessage, getRequestErrorMessage(error, "We couldn't submit your request. Please try again."), "error");
    } finally {
...
function openDetailsModal(elements, request) {
  ...
  if (window.bootstrap) {
    window.bootstrap.Modal.getOrCreateInstance(elements.detailsModal).show();
  } else {
...
function closeDetailsModal(elements) {
  if (elements.detailsModal) {
    if (window.bootstrap) {
      window.bootstrap.Modal.getOrCreateInstance(elements.detailsModal).hide();
    } else {
...
function renderTable(state, elements) {
  ...
  sorted.forEach(function (job) {
    ...
    statusBadge.className = "status-badge badge rounded-pill status-" + job.status;
    ...
    actionGroup.className = "action-group d-flex flex-wrap gap-2";
...
function renderAvailableJobs(state, elements) {
  ...
  if (available.length === 0) {
    ...
    empty.className = "empty-state alert alert-light mb-0";
  ...
  available.forEach(function (job) {
    ...
    card.className = "available-job-card card border-0 shadow-sm";
    ...
    actionWrap.className = "available-action d-flex flex-wrap gap-2";

    actionWrap.appendChild(createActionButton("View", "action-view", "view", job.id, false));

    var button = document.createElement("button");
    button.type = "button";
    button.className = "available-accept-btn btn btn-primary btn-sm";
...
function renderPipeline(state, elements) {
  ...
  state.assignedJobs.forEach(function (job) {
    ...
    var card = document.createElement("article");
    card.className = "pipeline-card card border-0 shadow-sm status-" + job.status;
  ...
  elements.pipelineLists.forEach(function (list) {
    if (list.children.length === 0) {
      var empty = document.createElement("p");
      empty.className = "pipeline-empty text-body-secondary mb-0";
...
function renderNotifications(state, elements) {
  ...
  state.notifications.forEach(function (notification) {
    var card = document.createElement("article");
    card.className = "notification-card card border-0";
    ...
    meta.className = "notification-meta d-flex flex-wrap justify-content-between align-items-center gap-2";
    meta.appendChild(document.createTextNode(formatDate(notification.createdAt)));

    if (!notification.read) {
      var button = document.createElement("button");
      button.type = "button";
      button.className = "dashboard-button notification-read-btn btn btn-outline-success btn-sm";
...
function appendEmptyTableRow(tableBody, message) {
  ...
  var cell = document.createElement("td");
  cell.colSpan = 6;
  cell.className = "empty-state text-center text-body-secondary py-5";
...
function createActionButton(text, className, action, id, disabled) {
  var button = document.createElement("button");
  button.type = "button";
  button.className = "action-btn btn btn-sm " + (action === "view" ? "btn-outline-secondary " : "btn-outline-success ") + className;
...
function setFormMessage(element, message, type) {
  ...
  element.className = "form-message alert mb-0 " + (type === "success" ? "alert-success" : "alert-danger");
}
...
function clearFormMessage(element) {
  ...
  element.textContent = "";
  element.className = "form-message alert d-none mb-0";
}
